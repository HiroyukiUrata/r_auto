...
コメント管理画面に表示されるユーザーは、以下の条件に基づいて選定されます。

| 対象 | 条件 | UIアイコン |
| :--- | :--- | :--- |
| **コメント対象** | `comment_text` が存在し、かつ、まだそのコメントが投稿されていない（`last_commented_at` が `comment_generated_at` より古い、または `last_commented_at` が `NULL`）。 | 💬 |
| **いいね返しのみ対象** | 上記のコメント対象ではなく、かつ `recent_like_count` が3件以上ある。 | ❤️ |
...
- **古いコメントの取り消し線**: `comment_generated_at` <= `last_commented_at` の場合、そのコメントは「投稿済み」と判断され、取り消し線付きで表示される。
