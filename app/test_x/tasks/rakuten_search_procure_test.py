...
class RakutenSearchProcureTestTask(BaseTask):
    """
    【検証用】楽天市場を検索して商品を調達するタスク。
    ログインプロファイルは不要。
    """
    def __init__(self, count: int = 5):
        super().__init__(count=count)
        self.action_name = "【検証用】楽天市場から商品を検索・調達"
...
def run_rakuten_search_procure_test(count: int = 5):
    """ラッパー関数"""
    task = RakutenSearchProcureTestTask(count=count)
    return task.run()