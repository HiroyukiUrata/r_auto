...
class FollowTestTask(BaseTask):
    """
    【検証用】楽天ROOMのユーザーを検索し、「フォロー」アクションを実行する。
    """
    def __init__(self, count: int = 10):
        super().__init__(count=count)
        self.action_name = "【検証用】フォロー"
...
def run_follow_test(count: int = 10):
    """ラッパー関数"""
    task = FollowTestTask(count=count)
    return task.run()