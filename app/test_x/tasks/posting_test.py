...
class PostingTestTask(BaseTask):
    """
    【検証用】ROOMへ商品情報を自動投稿するタスク。
    """
    def __init__(self, count: int = 10, product_id: int = None):
        # BaseTaskのcountはログ表示用
        super().__init__(count=count)
        self.action_name = "【検証用】記事投稿"
...
def run_posting_test(count: int = 10, product_id: int = None):
    """ラッパー関数"""
    task = PostingTestTask(count=count, product_id=product_id)
    return task.run()