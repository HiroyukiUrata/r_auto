...
class CreateCaptionTestTask:
    """
    【検証用】Geminiを使い投稿文を生成するタスク。
    このタスクはログインプロファイルを使わないため、BaseTaskを継承せず、
    独自のPlaywrightインスタンス管理を行う。
    """
    def __init__(self, count: int = 0):
        self.action_name = "【検証用】投稿文作成 (Gemini)"
...
def run_create_caption_test(count: int = 0):
    """ラッパー関数"""
    task = CreateCaptionTestTask(count=count)
    return task.run()